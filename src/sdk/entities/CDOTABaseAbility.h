...
public:
	...
	T GetLevelSpecialValueFor(std::string_view value, int level = -1) const {
		return GetVFunc(VMI::CDOTABaseAbility::GetLevelSpecialValueFor).Call<T>(value.data(), level, nullptr, 0, nullptr);
	}

	GETTER(bool, IsToggled, 0x59d); // For things like Pudge's Rot or Armlet
	GETTER(bool, IsHidden, 0x587);
	GETTER(float, GetCooldown, 0x5a8);
	GETTER(float, GetCooldownLength, 0x5ac);
	GETTER(int, GetLevel, 0x598);
	GETTER(int, GetCharges, 0x5cc);
	GETTER(float, GetChargeRestoreCooldown, 0x5d0);
	GETTER(bool, IsInAbilityPhase, 0x5a4);
